...
library(readxl)
library(writexl)
df_original <- read_excel("09.08.2021.xlsx")

df <- df_original[-c(1:11, 132:140)]
...
for (i in 1:ilosc_rzedow){
  ...
  if(is.na(kwantyl_25)){
    ...
    if(kwantyl_25 == 0 & kwantyl_75 == 0){
      ilosc_cyfr_przed <- length(koncowy_wektor_sprzedazy)
      wektor_pozycji_do_usuniêcia_z_faktur <- which(koncowy_wektor_sprzedazy == 0)
      koncowy_wektor_sprzedazy <- koncowy_wektor_sprzedazy[koncowy_wektor_sprzedazy != 0]
      koncowy_wektor_faktur <- koncowy_wektor_faktur[-wektor_pozycji_do_usuniêcia_z_faktur]
      ilosc_cyfr_po <- length(koncowy_wektor_sprzedazy)
      ilosc_zer <- ilosc_cyfr_przed - ilosc_cyfr_po
      ...
      if(is.na(kwantyl_25)){
        ...
        if ((length(koncowy_wektor_sprzedazy)+ilosc_zer)<5){
          do_macierzy <- append(do_macierzy, c(index, '0'))
          d <- d+1
          e <- e+1
          counter_rzedow <- counter_rzedow+1
        }else{
          do_macierzy <- append(do_macierzy, c(index, sum(czysty_wektor)/(length(czysty_wektor)+ilosc_zer)))
          d <- d+1
          e <- e+1
          counter_rzedow <- counter_rzedow+1
        }
